...
また、**Google フォトからダウンロードした際のメタデータ JSON（`*.json`）からのタイムスタンプ自動復元**、および **公式アプリで既に処理されて傾いてしまった動画の姿勢回転再補正** にも対応しています。
...
2. **公式アプリで既に処理して傾いてしまった動画の再補正**
   * 元の未加工動画（Dual-Fisheye）が手元になく、公式アプリで処理済みの傾いた Equirectangular 動画しかない場合でも、`-FixPitch`, `-FixRoll`, `-FixYaw` パラメータ（FFmpeg `v360` フィルタ）により、劣化を最小限に抑えて完全水平に回転再補正できます。

3. **Google フォト JSON からの撮影日時・更新日時 自動復元**
...
4. **4ch 空間音声（AmbiX + SA3D）の FLAC（可逆圧縮）自動展開**
...
### 方法 1: ドラッグ＆ドロップ (おすすめ)
1. 変換したい動画（`.MP4` / `.MOV`）または静止画（`.JPG`）を **`Convert-ThetaVideo.bat`** にドラッグ＆ドロップします（複数ファイル選択可）。
...
# 公式アプリで既に処理して傾いてしまった動画の姿勢回転再補正
.\Convert-ThetaVideo.ps1 .\R0010414_er.MP4 -FixPitch -3.0 -FixRoll 3.5 -NonInteractive
